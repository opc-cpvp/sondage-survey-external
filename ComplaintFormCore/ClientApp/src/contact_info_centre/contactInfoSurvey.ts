...
export class ContactInfoSurvey extends SurveyBase {
    ...
    private handleOnServerValidateQuestions(sender: SurveyModel, options: any): void {
        ...
        const validationUrl = `/api/ContactInfoSurvey/Validate?complaintId=${this.authToken}`;
    ...
    private handleOnComplete(sender: SurveyModel, options: any): void {
        void (async () => {
            const completeUrl = `/api/ContactInfoSurvey/Complete?complaintId=${this.authToken}`;
